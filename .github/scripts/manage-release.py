...
def load_config() -> dict[str, Any]:
    ...
    ignored: list[str] = []
    for path in allowed:
        result = run(["git", "check-ignore", "--quiet", "--", path], check=False)
        if result.returncode == 0:
            ignored.append(path)
        elif result.returncode != 1:
            fail(f"cannot determine whether release path is committable: {path}")
    if ignored:
        fail(
            "release configuration contains ignored paths that cannot be committed: "
            + ", ".join(ignored)
        )
    return value
