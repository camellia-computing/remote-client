...
verify_archive() {
	archive_path="$1"
	archive_sha256=$(echo "$VERSION_MANIFEST" | jq -r '.sha256 // empty')
	if [[ ! "$archive_sha256" =~ ^[0-9a-fA-F]{64}$ ]]; then
		echo "Flutter release manifest does not contain a valid SHA-256 digest"
		exit 1
	fi
...
download_archive() {
	...
	curl \
		...
		--connect-timeout 15 \
		--retry 5 \
		--output "$archive_local" \
		"$archive_url"
	verify_archive "$archive_local"
...
while getopts 'tc:k:d:l:pa:n:f:g:' flag; do
	...
	f)
		...
		if [ -n "$VERSION_FILE" ]; then
			if [[ "$VERSION_FILE" != *".fvmrc" ]] \
				&& [[ "$VERSION_FILE" != *"fvm_config.json" ]] \
				&& ! check_command yq; then
...
expand_key() {
	...
	version_sha256=$(echo "$VERSION_MANIFEST" | jq -r '.sha256')
	...
	expanded_key="${expanded_key/:sha256:/$version_sha256}"
