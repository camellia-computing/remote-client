...
- The `res/strategies.py`, `res/device-groups.py`, and `res/users.py` batch-control commands send a canonical UUID `Idempotency-Key`, print it before the mutation, and accept `--operation-id` to recover a lost response within Management's configured receipt-retention window. They report success only when the receipt binds the exact operation ID and canonical request digest and its typed `requested`/`applied` counts match every requested target; partial, legacy, malformed, missing-group, and zero-user force-logout results exit nonzero.
