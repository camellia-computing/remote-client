...
use camellia_remote_protocol::{message_proto::MultiClipboards, protobuf::Message};
...
fn try_init_rustls_platform_verifier(env: &mut JNIEnv, context_jobject: *mut c_void) {
    use camellia_remote_protocol::config::ANDROID_RUSTLS_PLATFORM_VERIFIER_INITIALIZED as INITIALIZED;
    ...
    if !initialized {
        ...
        if let Err(e) = camellia_remote_protocol::rustls_platform_verifier::android::init_hosted(
            env,
            ctx_for_rustls,
        ) {
