...
#define RUSTDESK_LIB_PATH "camellia_remote.so"
typedef bool (*CamelliaRemoteCoreMain)();
bool gIsConnectionManager = false;

void print_help_install_pkg(const char* so);

bool flutter_camellia_remote_core_main() {
   void* camellia_remote = dlopen(RUSTDESK_LIB_PATH, RTLD_LAZY);
   if (!camellia_remote) {
      fprintf(stderr,"Failed to load \"camellia_remote.so\"\n");
      char* error;
   ...
   auto core_main = (CamelliaRemoteCoreMain) dlsym(camellia_remote,"camellia_remote_core_main");
   char* error;
   if ((error = dlerror()) != nullptr) {
       fprintf(stderr, "Program entry \"camellia_remote_core_main\" is not found: %s\n", error);
       return false;
   }
   return core_main();
}
...
int main(int argc, char** argv) {
  if (!flutter_camellia_remote_core_main()) {
      return 0;
  }
