...
import 'package:camellia_remote_app/desktop/widgets/refresh_wrapper.dart';
...
import 'package:camellia_remote_app/native/common.dart'
    if (dart.library.html) 'package:camellia_remote_app/web/common.dart';
...
// rust: libs/camellia_remote_protocol/src/config.rs -> option2bool()
  ...
  if (uri.authority.isEmpty &&
    ...
    if (isAndroid || isIOS) {
      ...
      if (!allowDeepLinkServerSettings) {
        debugPrint(
          "Ignore camellia-remote://config because $kOptionAllowDeepLinkServerSettings is not enabled.",
        );
    ...
    if (isAndroid || isIOS) {
      ...
      if (!allowDeepLinkPassword) {
        debugPrint(
          "Ignore camellia-remote://password because $kOptionAllowDeepLinkPassword is not enabled.",
        );
        // Keep the user-facing error generic; detailed rejection reason is in debug logs.
        // Delay toast to avoid missing overlay during cold-start deeplink handling.
        Timer(Duration(seconds: 1), () {
    ...
    // camellia-remote://<connect-id>
    // camellia-remote://<connect-id>/r
    // camellia-remote://<connect-id>/r@<server>
    command = '--connect';
    id = uri.authority;
    if (uri.path.length > 1) {
      id = id + uri.path;
...
const kMacOSPermChannel = MethodChannel("com.camellia.remote/host");
...
Widget loadPowered(BuildContext context) {
  ...
  return MouseRegion(
    ...
    child: GestureDetector(
      onTap: () {
        launchUrl(
          Uri.parse('https://github.com/camellia-computing/remote-client'),
        );
      },
      child: Opacity(
        opacity: 0.5,
...
bool whitelistNotEmpty() {
  // https://github.com/camellia-computing/remote-client/blob/main/README.md
