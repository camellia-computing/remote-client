...
use camellia_remote_protocol::{message_proto::CursorData, sysinfo::Pid, ResultType};
...
impl WakeLock {
    pub fn new(tag: &str) -> Self {
        ...
        match android_wakelock::partial(tag) {
            ...
            Err(e) => {
                camellia_remote_protocol::log::error!("Failed to get wakelock: {e:?}");
                Self::default()
            }
        }
    }
...
pub fn get_wakelock(_display: bool) -> WakeLock {
    camellia_remote_protocol::log::info!("new wakelock, require display on: {_display}");
    #[cfg(target_os = "android")]
