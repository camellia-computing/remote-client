...
pub fn get_custom_client_staging_dir() -> PathBuf {
    get_public_base_dir()
        .join("Camellia Remote")
        .join("camellia-remote-staging")
}
...
pub fn create_shortcut(id: &str) -> ResultType<()> {
    ...
    // https://github.com/rustdesk/camellia_remote_protocol/blob/8b0e25867375ba9e6bff548acf44fe6d6ffa7c0e/src/config.rs#L1384
...
pub fn is_elevated(process_id: Option<DWORD>) -> ResultType<bool> {
    use camellia_remote_protocol::platform::windows::RAIIHandle;
    unsafe {
    ...
    for entry in entries {
        if let Ok(entry) = entry {
            ...
            if let Some(file_name) = path.file_name().and_then(|n| n.to_str()) {
                // Match Camellia Remote installers downloaded by the updater.
                if file_name.starts_with("camellia-remote-")
...
) -> ResultType<Vec<camellia_remote_protocol::sysinfo::Pid>> {
...
fn get_pids_with_args_from_wmic_output<S2: AsRef<str>>(
    ...
    name: &str,
    args: &[S2],
) -> Vec<camellia_remote_protocol::sysinfo::Pid> {
    // CommandLine=
    // ProcessId=33796
    //
    // CommandLine=
        ...
        if line.starts_with("ProcessId=") {
            if proc_found {
                if let Ok(pid) = line["ProcessId=".len()..].trim().parse::<u32>() {
                    pids.push(camellia_remote_protocol::sysinfo::Pid::from_u32(pid));
                }
                proc_found = false;
            }
...
mod tests {
    ...
    //
    // This struct is similar to `camellia_remote_protocol::platform::windows::RAIIHandle`,
