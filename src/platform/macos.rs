...
type BooleanT = camellia_remote_protocol::libc::c_int;
...
fn get_update_temp_dir() -> PathBuf {
    let euid = unsafe { camellia_remote_protocol::libc::geteuid() };
        ...
        while running.load(Ordering::SeqCst) {
            ...
            if tmp != uid && !tmp.is_empty() {
                ...
                if let Some(ps) = server.as_mut() {
                    camellia_remote_protocol::allow_err!(ps.kill());
                }
            }
            if let Some(ps) = server.as_mut() {
                match ps.try_wait() {
...
pub fn update_me() -> ResultType<()> {
    ...
    let is_service_stopped = camellia_remote_protocol::config::option2bool(
        option_stop_service,
...
fn update_extracted(target_dir: &str) -> ResultType<()> {
    ...
    let _child = unsafe {
        if let Err(e) = Command::new(&exe_path)
            ...
            .stderr(Stdio::null())
            .pre_exec(|| {
                camellia_remote_protocol::libc::setsid();
                Ok(())
            })
            .spawn()
        {
...
pub fn handle_application_should_open_untitled_file() {
    camellia_remote_protocol::log::debug!("icon clicked on finder");
