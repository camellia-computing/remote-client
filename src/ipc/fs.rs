...
use camellia_remote_protocol::{config, log, ResultType};
use std::{
    ffi::CString,
    io::{Error, ErrorKind},
...
impl Drop for FdGuard {
    fn drop(&mut self) {
        unsafe {
            camellia_remote_protocol::libc::close(self.0);
        }
    }
}
...
fn open_ipc_parent_dir_fd(parent_c: &CString) -> std::io::Result<i32> {
    let fd = unsafe {
        camellia_remote_protocol::libc::open(
            parent_c.as_ptr(),
            camellia_remote_protocol::libc::O_RDONLY
                | camellia_remote_protocol::libc::O_DIRECTORY
...
fn remove_parent_entry_via_fd(
    ...
    let mut stat: camellia_remote_protocol::libc::stat = unsafe { std::mem::zeroed() };
    let stat_rc = unsafe {
        camellia_remote_protocol::libc::fstatat(
            parent_fd,
            entry_c.as_ptr(),
            &mut stat,
            camellia_remote_protocol::libc::AT_SYMLINK_NOFOLLOW,
        )
    };
    if stat_rc != 0 {
    ...
    let is_dir = (stat.st_mode
        & (camellia_remote_protocol::libc::S_IFMT as camellia_remote_protocol::libc::mode_t))
        == camellia_remote_protocol::libc::S_IFDIR;
    let unlink_flags = if is_dir {
        camellia_remote_protocol::libc::AT_REMOVEDIR
    } else {
        0
    };
    let unlink_rc = unsafe {
        camellia_remote_protocol::libc::unlinkat(parent_fd, entry_c.as_ptr(), unlink_flags)
    };
    if unlink_rc != 0 {
        let err = std::io::Error::last_os_error();
        if err.kind() == ErrorKind::NotFound {
...
pub(crate) fn ensure_secure_ipc_parent_dir(path: &str, postfix: &str) -> ResultType<bool> {
    ...
    let fd = match open_ipc_parent_dir_fd(&parent_c) {
        ...
        Err(open_err) => {
            ...
            if open_err.raw_os_error() == Some(camellia_remote_protocol::libc::ENOENT) {
                let expected_mode = expected_ipc_parent_mode(postfix);
                let rc = unsafe {
                    camellia_remote_protocol::libc::mkdir(
                        parent_c.as_ptr(),
                        expected_mode as camellia_remote_protocol::libc::mode_t,
                    )
                };
                if rc != 0 {
                    let mkdir_err = std::io::Error::last_os_error();
                    // Handle a race where another process created the directory first.
                    if mkdir_err.raw_os_error() != Some(camellia_remote_protocol::libc::EEXIST) {
                        return Err(Error::new(
                            mkdir_err.kind(),
                            format!(
    ...
    let mut st: camellia_remote_protocol::libc::stat = unsafe { std::mem::zeroed() };
    if unsafe { camellia_remote_protocol::libc::fstat(fd, &mut st as *mut _) } != 0 {
    ...
    let is_dir = (mode & (camellia_remote_protocol::libc::S_IFMT as u32))
        == (camellia_remote_protocol::libc::S_IFDIR as u32);
    if !is_dir {
        return Err(Error::new(
    ...
    let expected_uid = unsafe { camellia_remote_protocol::libc::geteuid() as u32 };
    ...
    if owner_uid != expected_uid && expected_uid == 0 && config::is_service_ipc_postfix(postfix) {
        let rc = unsafe {
            camellia_remote_protocol::libc::fchown(
                fd,
                expected_uid as camellia_remote_protocol::libc::uid_t,
                camellia_remote_protocol::libc::gid_t::MAX,
            )
        };
        if rc == 0 {
            let mut st2: camellia_remote_protocol::libc::stat = unsafe { std::mem::zeroed() };
            if unsafe { camellia_remote_protocol::libc::fstat(fd, &mut st2 as *mut _) } == 0 {
    ...
    if repaired_parent_mode {
        ...
        if unsafe {
            camellia_remote_protocol::libc::fchmod(
                fd,
                expected_mode as camellia_remote_protocol::libc::mode_t,
            )
        } != 0
...
fn write_pid_file(path: &Path) -> ResultType<()> {
    ...
    let flags = camellia_remote_protocol::libc::O_WRONLY
        | camellia_remote_protocol::libc::O_CREAT
        | camellia_remote_protocol::libc::O_CLOEXEC
        | camellia_remote_protocol::libc::O_NOFOLLOW
        | camellia_remote_protocol::libc::O_NONBLOCK;
    let fd = unsafe { camellia_remote_protocol::libc::open(path_c.as_ptr(), flags, 0o0600) };
    if fd < 0 {
        let os_err = std::io::Error::last_os_error();
        return Err(Error::new(
            os_err.kind(),
    ...
    let _fd_guard = FdGuard(fd);
    let mut stat: camellia_remote_protocol::libc::stat = unsafe { std::mem::zeroed() };
    if unsafe { camellia_remote_protocol::libc::fstat(fd, &mut stat) } != 0 {
        let os_err = std::io::Error::last_os_error();
        return Err(Error::new(
            os_err.kind(),
    ...
    if (stat.st_mode
        & (camellia_remote_protocol::libc::S_IFMT as camellia_remote_protocol::libc::mode_t))
        != (camellia_remote_protocol::libc::S_IFREG as camellia_remote_protocol::libc::mode_t)
    {
        return Err(Error::new(
            ErrorKind::PermissionDenied,
    ...
    if unsafe { camellia_remote_protocol::libc::fchmod(fd, 0o600) } != 0 {
        let os_err = std::io::Error::last_os_error();
        return Err(Error::new(
            os_err.kind(),
    ...
    if unsafe { camellia_remote_protocol::libc::ftruncate(fd, 0) } != 0 {
        let os_err = std::io::Error::last_os_error();
        return Err(Error::new(
            os_err.kind(),
            format!(
    ...
    while written < buf.len() {
        let rc = unsafe {
            camellia_remote_protocol::libc::write(
                fd,
                buf[written..].as_ptr() as *const camellia_remote_protocol::libc::c_void,
                buf.len() - written,
            )
        };
        if rc < 0 {
...
fn read_pid_file_secure(path: &Path) -> Option<usize> {
    ...
    let flags = camellia_remote_protocol::libc::O_RDONLY
        | camellia_remote_protocol::libc::O_CLOEXEC
        | camellia_remote_protocol::libc::O_NOFOLLOW
        | camellia_remote_protocol::libc::O_NONBLOCK;
    let fd = unsafe { camellia_remote_protocol::libc::open(path_c.as_ptr(), flags) };
    if fd < 0 {
        return None;
    }
    let _fd_guard = FdGuard(fd);
    ...
    let read_len = unsafe {
        camellia_remote_protocol::libc::read(
            fd,
            buffer.as_mut_ptr() as *mut camellia_remote_protocol::libc::c_void,
            buffer.len(),
        )
    };
    if read_len <= 0 {
...
pub(crate) async fn check_pid(postfix: &str) -> bool {
    ...
    if let Some(pid) = read_pid_file_secure(&pid_file) {
        if pid > 0 {
            let mut sys = camellia_remote_protocol::sysinfo::System::new();
