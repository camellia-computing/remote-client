use camellia_remote_protocol::{
    bail, platform::windows::is_windows_version_or_greater, ResultType,
};

// This string is defined here.
// The adapter description must remain aligned with the bundled indirect-display driver INF.
pub const CAMELLIA_REMOTE_IDD_DEVICE_STRING: &'static str = "CamelliaIddDriver Device\0";
...
pub fn get_cur_device_string() -> &'static str {
    match IDD_IMPL {
        IDD_IMPL_CAMELLIA => CAMELLIA_REMOTE_IDD_DEVICE_STRING,
...
pub mod rustdesk_idd {
    use super::windows;
    use camellia_remote_protocol::{allow_err, bail, lazy_static, log, ResultType};
    use std::{
    ...
    fn get_device_names() -> Vec<String> {
        windows::get_device_names(Some(super::CAMELLIA_REMOTE_IDD_DEVICE_STRING))
    }
...
pub mod amyuni_idd {
    ...
    use camellia_remote_protocol::{bail, lazy_static, log, tokio::time::Instant, ResultType};
    use std::{
        ptr::null_mut,
