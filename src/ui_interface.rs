...
use camellia_remote_protocol::password_security;
...
pub fn test_if_valid_server(host: String, test_with_proxy: bool) -> String {
    camellia_remote_protocol::socket_client::test_if_valid_server(&host, test_with_proxy)
}
...
pub fn get_uuid() -> String {
    crate::encode64(camellia_remote_protocol::get_uuid())
}
...
pub fn set_user_default_option(key: String, value: String) {
    use camellia_remote_protocol::config::UserDefaultConfig;
...
pub async fn change_id_shared_(id: String, old_id: String) -> &'static str {
    if !camellia_remote_protocol::is_valid_custom_id(&id) {
        log::debug!(
        ...
        log::debug!(
            "bom: {}",
            camellia_remote_protocol::is_valid_custom_id(&bom)
        );
    ...
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    let uuid = Bytes::from(
        camellia_remote_protocol::machine_uid::get()
            .unwrap_or("".to_owned())
            .as_bytes()
            .to_vec(),
    );
    #[cfg(any(target_os = "android", target_os = "ios"))]
    let uuid = Bytes::from(camellia_remote_protocol::get_uuid());
...
async fn check_id(
    ...
    if let Ok(mut socket) = camellia_remote_protocol::socket_client::connect_tcp(
...
pub fn max_encrypt_len() -> usize {
    camellia_remote_protocol::config::ENCRYPT_MAX_LEN
}
