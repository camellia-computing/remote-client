...
pub fn core_main() -> Option<Vec<String>> {
    ...
    if args.is_empty() {
        ...
        if should_check_start_tray && !crate::check_process("--tray", true) {
            #[cfg(target_os = "linux")]
            camellia_remote_protocol::allow_err!(crate::platform::check_autostart_config());
    ...
    if args.contains(&"--connect".to_string()) || args.contains(&"--view-camera".to_string()) {
        camellia_remote_protocol::platform::windows::start_cpu_performance_monitor();
    }
    #[cfg(feature = "flutter")]
    ...
    camellia_remote_protocol::init_log(false, &log_name);
    ...
    if args.is_empty() || crate::common::is_empty_uni_link(&args[0]) {
        ...
        #[cfg(windows)]
        {
            ...
            camellia_remote_protocol::config::PeerConfig::preload_peers();
        }
        ...
        #[cfg(windows)]
        {
            ...
            if args[0] == "--uninstall" {
                ...
                #[cfg(windows)]
                camellia_remote_protocol::allow_err!(crate::platform::windows::uninstall_cert());
                return None;
        ...
        if args[0] == "--remove" {
            ...
            #[cfg(target_os = "macos")]
            {
                ...
                camellia_remote_protocol::allow_err!(handler.join());
            }
            return None;
...
fn import_config(path: &str) {
    use camellia_remote_protocol::{config::*, get_exe_time, get_modified_time};
