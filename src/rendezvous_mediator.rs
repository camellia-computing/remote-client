...
impl RendezvousMediator {
    ...
    pub async fn start_tcp(server: ServerPtr, host: String) -> ResultType<()> {
        ...
        log::info!(
            "start tcp: {}",
            camellia_remote_protocol::websocket::check_ws(&host)
        );
    ...
    async fn handle_intranet(&self, fla: FetchLocalAddr, server: ServerPtr) -> ResultType<()> {
        ...
        *LAST_MSG.lock().await = (addr, Instant::now());
        ...
        let peer_addr_v6 = camellia_remote_protocol::AddrMangle::decode(&fla.socket_addr_v6);
    ...
    async fn handle_punch_hole(&self, ph: PunchHole, server: ServerPtr) -> ResultType<()> {
        ...
        *LAST_MSG.lock().await = (peer_addr, Instant::now());
        ...
        let peer_addr_v6 = camellia_remote_protocol::AddrMangle::decode(&ph.socket_addr_v6);
    ...
    async fn punch_udp_hole(
        ...
        tokio::spawn(async move {
            for _ in 0..2 {
                let tm = (camellia_remote_protocol::time_based_rand() % 20 + 10) as f32 / 1000.;
                camellia_remote_protocol::sleep(tm).await;
    ...
    loop {
        ...
        if let Some(l) = listener.as_mut() {
            ...
            if let Ok(Ok((stream, addr))) =
                camellia_remote_protocol::timeout(1000, l.accept()).await
            {
                ...
                tokio::spawn(async move {
                    allow_err!(
                        crate::server::create_tcp_connection(
                            server,
                            camellia_remote_protocol::Stream::from(stream, local_addr),
                            addr,
                            false,
