...
#[cfg(target_os = "android")]
use camellia_remote_protocol::protobuf::EnumOrUnknown;
...
impl Connection {
    ...
    async fn send_logon_response_and_keep_alive(&mut self) -> bool {
        ...
        #[cfg(not(target_os = "android"))]
        {
            ...
            pi.platform = camellia_remote_protocol::whoami::platform().to_string();
        }
        #[cfg(target_os = "android")]
        {
        ...
        #[cfg(any(target_os = "windows", feature = "unix-file-copy-paste"))]
        {
            let is_both_windows = cfg!(target_os = "windows")
                && self.lr.my_platform
                    == camellia_remote_protocol::whoami::Platform::Windows.to_string();
            #[cfg(feature = "unix-file-copy-paste")]
    ...
    fn permission(
        ...
        use camellia_remote_protocol::rendezvous_proto::control_permissions::Permission;
    ...
    async fn on_message(&mut self, msg: Message) -> bool {
        ...
        if let Some(message::Union::LoginRequest(lr)) = msg.union {
                ...
                || password::approve_mode() == ApproveMode::Both && !password::has_valid_password()
            {
                ...
                if camellia_remote_protocol::get_version_number(&lr.version)
                    >= camellia_remote_protocol::get_version_number("1.2.0")
                {
                    ...
                    if res {
                        ...
                        if !tfa.hwid.is_empty() && Self::enable_trusted_devices() {
                            Config::add_trusted_device(TrustedDevice {
                                hwid: tfa.hwid,
                                time: camellia_remote_protocol::get_time(),
                                id: self.lr.my_id.clone(),
                                name: self.lr.my_name.clone(),
    ...
    fn handle_administrator_check(
        ...
        let check_admin_res =
            crate::platform::get_logon_user_token(username, password).map(|token| {
                ...
                unsafe {
                    camellia_remote_protocol::allow_err!(CloseHandle(HANDLE(token as _)));
                };
                is_token_admin
            });
    ...
    // note: we specifically don't use camellia_remote_protocol::is_ipv6_str to avoid divergence issues
    ...
    async fn update_options(&mut self, o: &OptionMessage) {
        ...
        if camellia_remote_protocol::get_version_number(&self.lr.version)
            < camellia_remote_protocol::get_version_number("1.2.4")
        {
    ...
    fn turn_off_privacy_result_to_msg(
        turn_off_res: Option<camellia_remote_protocol::ResultType<()>>,
        impl_key: String,
    ) -> Message {
        match turn_off_res {
    ...
    fn is_bool_option_not_set(
        option: camellia_remote_protocol::protobuf::EnumOrUnknown<BoolOption>,
    ) -> bool {
    ...
    async fn handle_file_clip(&mut self, clip: clipboard::ClipboardFile) {
        ...
        if !stop {
            use camellia_remote_protocol::config::keys::OPTION_ONE_WAY_FILE_TRANSFER;
...
async fn start_ipc(
    ...
    use camellia_remote_protocol::anyhow::anyhow;
    ...
    if stream.is_none() {
        ...
        if headless_cm {
            ...
            let uid = {
                ...
                let mut uid_cmd = camellia_remote_protocol::tokio::process::Command::new("id");
                // TODO:
...
impl Drop for Connection {
    fn drop(&mut self) {
        ...
        if let Some(TerminalUserToken::CurrentLogonUser(token)) = self.terminal_user_token.take() {
            if token.as_raw() != 0 {
                unsafe {
                    camellia_remote_protocol::allow_err!(CloseHandle(HANDLE(token.as_raw() as _)));
                };
            }
        }
    }
...
mod test {
    ...
    fn set_supported_decoding(option: &mut OptionMessage) {
        option.supported_decoding =
            camellia_remote_protocol::protobuf::MessageField::some(Default::default());
    }
                    ...
                    (
                        option_msg(|o| {
                            o.supported_decoding =
                                camellia_remote_protocol::protobuf::MessageField::some(
                                    Default::default(),
                                )
                        }),
                        None,
