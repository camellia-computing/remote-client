...
mod pa_impl {
    ...
    enum Aligned32 {
        ...
        Native(Vec<u8>),
        Realigned(camellia_remote_protocol::mem::AlignedU8Vec),
    }
    ...
    fn align_to_32(data: Vec<u8>) -> Aligned32 {
        ...
        let mut buf = camellia_remote_protocol::mem::aligned_u8_vec(data.len(), 4);
    ...
    pub async fn run(sp: EmptyExtraFieldService) -> ResultType<()> {
        camellia_remote_protocol::sleep(0.1).await; // one moment to wait for _pa ipc
        ...
        while sp.ok() && !RESTARTING.load(Ordering::SeqCst) {
            ...
            if scrap::android::ffi::get_audio_raw(&mut android_data, &mut vec![]).is_some() {
                ...
                camellia_remote_protocol::sleep(0.1).await;
            }
        }
        Ok(())
    }
