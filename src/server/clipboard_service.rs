...
use camellia_remote_protocol::config::{keys, option2bool};
            ...
            if !urls.is_empty() {
                ...
                match clipboard::platform::unix::serv_files::sync_files(&urls) {
                    Ok(()) => {
                        ...
                        camellia_remote_protocol::allow_err!(clipboard::send_data(
                            0,
                    ...
                    Some(Data::ClipboardNonFile(Some((err, mut contents)))) => {
                        if !err.is_empty() {
                            ...
                            if contents.iter().any(|c| c.next_raw) {
                                ...
                                // thread '<unnamed>' panicked at D:\Projects\rust\rustdesk\libs\camellia_remote_protocol\src\lib.rs:98:5:
