...
pub fn start_tray() {
    if crate::ui_interface::get_builtin_option(
        camellia_remote_protocol::config::keys::OPTION_HIDE_TRAY,
    ) == "Y"
    {
...
fn make_tray() -> camellia_remote_protocol::ResultType<()> {
    // https://github.com/tauri-apps/tray-icon/blob/dev/examples/tao.rs
    use camellia_remote_protocol::anyhow::Context;
    ...
    let hide_stop_service = crate::ui_interface::get_builtin_option(
        camellia_remote_protocol::config::keys::OPTION_HIDE_STOP_SERVICE,
    ) == "Y";
...
async fn start_query_session_count(sender: std::sync::mpsc::Sender<Data>) {
    ...
    loop {
        ...
        camellia_remote_protocol::sleep(1.).await;
    }
}
