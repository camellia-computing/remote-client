...
use camellia_remote_protocol::{bail, config, log, ResultType};
use std::{
    io::Write,
...
fn acquire_mac_update_lock() -> ResultType<MacUpdateLock> {
    let path = std::path::PathBuf::from("/var/run/camellia-remote-update.lock");
    let handle = std::fs::OpenOptions::new()
        .read(true)
        .write(true)
        .create(true)
        .custom_flags(
            camellia_remote_protocol::libc::O_NOFOLLOW | camellia_remote_protocol::libc::O_CLOEXEC,
        )
        .open(&path)?;
    ...
    let lock_result = unsafe {
        camellia_remote_protocol::libc::flock(
            handle.as_raw_fd(),
            camellia_remote_protocol::libc::LOCK_EX | camellia_remote_protocol::libc::LOCK_NB,
        )
    };
    if lock_result != 0 {
        ...
        if let Some(session_id) = crate::platform::get_current_process_session_id() {
            if update_msi {
                ...
                let custom_client_staging_dir = if crate::is_custom_client() {
                    ...
                    camellia_remote_protocol::allow_err!(
                        crate::platform::remove_custom_client_staging_dir(&staging_dir)
                    );
                    None
                };
                ...
                if !update_launched {
                    if let Some(dir) = custom_client_staging_dir {
                        camellia_remote_protocol::allow_err!(
                            crate::platform::remove_custom_client_staging_dir(&dir)
                        );
                    }
                    std::fs::remove_file(&file_path).ok();
                }
...
pub fn has_no_active_conns_ipc() -> bool {
    let rt = match camellia_remote_protocol::tokio::runtime::Runtime::new() {
        Ok(rt) => rt,
        Err(_) => return false,
    };
    rt.block_on(async {
...
pub fn check_update_as_root() -> ResultType<bool> {
    ...
    let dmg_url = format!("{}/camellia-remote-{}-{}.dmg", download_url, version, arch);
    log::info!(
    ...
    let private_tmp_output = std::process::Command::new("/usr/bin/mktemp")
        .args(["-d", "/tmp/.camellia-remote-download-XXXXXX"])
        .output()?;
    ...
    let private_tmp = String::from_utf8(private_tmp_output.stdout)
        .map_err(|err| {
            camellia_remote_protocol::anyhow::anyhow!("[root-update] mktemp output error: {}", err)
        })?
        .trim()
        .to_owned();
...
mod tests {
    ...
    fn update_download_file_accepts_expected_github_asset_urls() {
        let file = get_download_file_from_url(
            "https://github.com/camellia-computing/remote-client/releases/download/v0.1.0/camellia-remote-0.1.0-x86_64.dmg",
        )
        .expect("valid GitHub release asset URL");

        assert_eq!(
            file.file_name().and_then(|name| name.to_str()),
            Some("camellia-remote-0.1.0-x86_64.dmg")
        );
    }
