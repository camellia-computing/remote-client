...
use bytes::Bytes;
#[cfg(target_os = "linux")]
use camellia_remote_protocol::anyhow;
...
pub enum Data {
    ...
    /// See `libs/camellia_remote_protocol/src/bytes_codec.rs` test `test_codec2` for verification.
    FileBlockFromCM {
        id: i32,
        file_num: i32,
        ...
        Data::Close => {
            ...
            if EXIT_RECV_CLOSE.load(Ordering::SeqCst) {
                ...
                if crate::is_main() {
                    ...
                    #[cfg(target_os = "linux")]
                    {
                        camellia_remote_protocol::sleep(
                            (crate::platform::SERVICE_INTERVAL * 2) as f32 / 1000.0,
                        )
                        .await;
                    ...
                    #[cfg(target_os = "macos")]
                    {
                        ...
                        camellia_remote_protocol::sleep(1.5).await;
                    ...
                    camellia_remote_protocol::sleep(0.3).await;
        ...
        Data::Config((name, value)) => match value {
            None => {
                ...
                if name == "id" {
                    ...
                    {
                        Some(camellia_remote_protocol::password_security::hide_cm().to_string())
                    } else {
                        None
                    };
            ...
            allow_err!(
                stream
                    .send(&Data::SyncWinCpuUsage(
                        camellia_remote_protocol::platform::windows::cpu_uage_one_minute()
                    ))
                    .await
            );
        }
...
fn running_server_uids_for_current_exe() -> ResultType<Vec<u32>> {
    ...
    let current_pid = camellia_remote_protocol::sysinfo::Pid::from_u32(std::process::id());
...
pub async fn connect(ms_timeout: u64, postfix: &str) -> ResultType<ConnectionTmpl<ConnClient>> {
    #[cfg(any(target_os = "linux", target_os = "macos"))]
    {
        ...
        let is_root_main_ipc = unsafe { camellia_remote_protocol::libc::geteuid() == 0 }
            && postfix.is_empty()
            && use_user_main_ipc;
        if is_root_main_ipc {
...
pub async fn get_terminal_session_count() -> ResultType<usize> {
    ...
    let effective_uid = unsafe { camellia_remote_protocol::libc::geteuid() as u32 };
